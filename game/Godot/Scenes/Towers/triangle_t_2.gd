...
func fire():
	...
	shoot_sfx.play()
	apply_recoil()
	enemy.on_hit(GameData.tower_data["triangle_t_2"]["damage"])
	# Wait for the rof cooldown before being able to shoot again
	await get_tree().create_timer(GameData.tower_data["triangle_t_2"]["rof"]).timeout
	ready_to_shoot = true
...
func _ready():
	...
	if built:
		...
		placement_sfx.play()
		collision_shape.shape.radius = GameData.tower_data["triangle_t_2"]["range"] / 2
...
func upgrade(tower):
	print("upgraded")
	if tower == self:
		var upgraded_tower = load("res://Scenes/Towers/triangle_t_3.tscn")
