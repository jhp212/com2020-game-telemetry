...
func _physics_process(delta: float) -> void:
	if health < GameData.enemy_data["large_circle"]["health"] * 0.75 and health >= GameData.enemy_data["large_circle"]["health"] * 0.5:
		$Body/AnimatedSprite2D.play("75%health")
	elif health < GameData.enemy_data["large_circle"]["health"] * 0.5 and health >= GameData.enemy_data["large_circle"]["health"] * 0.25:
		$Body/AnimatedSprite2D.play("50%health")
	elif health < GameData.enemy_data["large_circle"]["health"] * 0.25 and health > 0:
