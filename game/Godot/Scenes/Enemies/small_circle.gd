...
var health =  GameData.enemy_data["small_circle"]["health"]
var damage =  GameData.enemy_data["small_circle"]["damage"]
...
func die(condition):
	...
	if condition == 0:
		...
		GameData.add_money(GameData.enemy_data["small_circle"]["cash"])
		Telemetry.log_event("enemy_defeated", {"enemy_id": 1})
...
func _physics_process(delta: float) -> void:
	if health < GameData.enemy_data["small_circle"]["health"] * 0.75 and health >= GameData.enemy_data["small_circle"]["health"] * 0.5:
		$Body/AnimatedSprite2D.play("75%health")
	elif health < GameData.enemy_data["small_circle"]["health"] * 0.5 and health >= GameData.enemy_data["small_circle"]["health"] * 0.25:
		$Body/AnimatedSprite2D.play("50%health")
	elif health < GameData.enemy_data["small_circle"]["health"] * 0.25 and health > 0:
