...
def within_bounds(tower_type, tower_location: tuple, path_location: tuple)  -> bool:
	match tower_type:
		...
		case 'square_stock':
			return (abs(tower_location[1] - path_location[1]) < 30) and (path_location[0] > tower_location[0])
		case 'star_stock':
			return False # I will need to update this with where the star can hit, but as of writing the star does not function
		case _:
			return False
...
def reset():
	global wavecount, base_health, base_money, enemies, waves, towers, tower_queue
	...
	for iteration in range(totaliterations): #stops infinte loops ^_^
		...
		for index,tower in enumerate(towers):
			...
			if total_hit == 0:
				...
				average_damage = tower_data[tower]['damage'] * player_rof_mult * timesamplerate/ (total_hit * tower_data[tower]['rof'])
