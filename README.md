# Simple Tower Defence Game with Telemetry and Balancing Toolkit

A simple tower defence game that records user telemetry data in order to facilitate the balancing of the game.

---

## Group Members

- Jake Phillips (Project Owner - Data Lead) @jhp212
- Oliver Robson (Scrum Master - Documentation) @O-Robson
- Doruk Aksu (Dev team - Tech lead) @doorukaksu
- Dylan Davies (Dev team - Software dev) @dd524
- Bela Roth (Dev team - QA & Testing) @brm457
- Thomas Wilkin-Jones (Dev team - Game dev) @twj64
- Zak Salmon (Dev team - Dashboard dev) @Salmonellaa

## Port Mappings

- 8000: Database Server
