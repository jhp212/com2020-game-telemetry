...
def test_balancing_rule(client):
    created = client.post("/balancing_rules/", json={
    ...
    r = client.get(f"/balancing_rules/?rule_id={created['id']}")
...
def test_create_decision_log(client):
    #first add a parameter to the DB, since decision log requires a parameter to exist
    client.post("/parameters/", json={"name": "enemy1_health", "value": 5.0})
    r = client.post("/decision_logs/", json={
...
def test_filter_decision_logs(client):
    client.post("/parameters/", json={"name": "enemy1_health", "value": 3.0})
    client.post("/parameters/", json={"name": "enemy_damage_multiplier", "value": 1.5})

    now = datetime.now(timezone.utc).isoformat()
    client.post("/decision_logs/", json={
    ...
    r = client.get("/decision_logs/?parameter_name=enemy_damage_multiplier")
    assert r.status_code == 200
    rows = r.json()
    assert len(rows) == 1
...
def test_create_parameter(client):
    r = client.post("/parameters/", json={"name": "enemy1_health", "value": 5.0})
    assert r.status_code == 200
...
def test_update_parameter(client):
    client.post("/parameters/", json={"name": "enemy1_health", "value": 5.0})
    r = client.post("/parameters/", json={"name": "enemy1_health", "value": 2.0})
    assert r.status_code == 200
    assert r.json()["value"] == 2.0

#tests filtering parameters by their name 
def test_filter_parameters(client):
    client.post("/parameters/", json={"name": "enemy1_health", "value": 1.0})
    client.post("/parameters/", json={"name": "enemy_damage_multiplier", "value": 1.5})

    r = client.get("/parameters/?parameter_name=enemy_damage_multiplier")
    assert r.status_code == 200
    rows = r.json()
    assert len(rows) == 1
...
def test_filter_telemetry(client):
    now = datetime.now(timezone.utc).isoformat()
    ...
    r = client.get("/telemetry/", params={"user_id": 2})
    assert r.status_code == 200, r.text
    rows = r.json()
    assert len(rows) == 1
